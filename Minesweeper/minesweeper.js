import { Field, Mine } from "./fields.js"
...
export class GameConfig {
    ...
    isValid() {
        return (
            this.fieldSize > this.MIN_FIELD_SIZE &&
...
function generateField(config) {
    ...
    while (placedMines < config.noOfMines) {
        for (let row = 0; row < config.fieldSize; row++) {
            const r = field[row] == null ? new Array(config.fieldSize) : field[row];
...
function handleFieldHit(fields, hitPosition, flagging) {
    function checkForVictory() {
        ...
        return (
            noOfFlaggedFields === 0 && noOfMines > 0 && noOfMines === noOfFlaggedMines
        );
    }
